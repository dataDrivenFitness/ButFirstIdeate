...
When you ask a question with specific options, format them clearly:
- "Are you targeting [iOS], [Android], or [both]?"
- "Is this for [businesses] or [consumers]?"
- "Would you charge [one-time], [subscription], or [free]?"

Put options in square brackets so the UI can make them tappable buttons.
...
export default function App() {
  ...
  const extractButtons = (text) => {
    // Look for patterns like: [option1], [option2], or [option3]?
    const bracketMatches = text.match(/\[([^\]]+)\]/g);
    if (bracketMatches && bracketMatches.length >= 2) {
      const options = bracketMatches.map(m => m.replace(/[\[\]]/g, ''));
      // Remove brackets from the text for display
      const cleanText = text.replace(/\[([^\]]+)\]/g, '$1');
      return { hasButtons: true, options, text: cleanText };
    }
    return { hasButtons: false, options: [], text };
  };
  ...
  // Get the last assistant message to check for buttons
  const lastAssistantMsg = [...messages].reverse().find(m => m.role === 'assistant');
  const buttonData = lastAssistantMsg ? extractButtons(lastAssistantMsg.content) : { hasButtons: false, options: [] };
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
          ...
          <div style={{ maxWidth: 600, margin: '0 auto' }}>
            {messages.map((m, i) => (
              <div key={i} style={{ display: 'flex', gap: 8, marginBottom: 12, flexDirection: m.role === 'user' ? 'row-reverse' : 'row' }}>
                ...
                }}>{m.role === 'assistant' && i === messages.length - 1 ? buttonData.text : m.content}</div>
              </div>
            ))}
            {isLoading && (
        ...
        <div style={{ padding: 12, background: 'white', borderTop: '1px solid #e5e7eb' }}>
          <div style={{ maxWidth: 600, margin: '0 auto' }}>
            ...
            {buttonData.hasButtons && !showCustomInput && !isLoading && (
              <div style={{ marginBottom: 12, display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                {buttonData.options.map((option, idx) => (
                  <button
                    key={idx}
            ...
            {/* Text input (always shown if no buttons, or if "Other" clicked) */}
            {(!buttonData.hasButtons || showCustomInput) && (
              <div style={{ display: 'flex', gap: 8 }}>
                <input 
                  value={input} 
                ...
                <button 
                  ...
                  style={{ 
                    padding: '10px 20px', 
                    background: isLoading || !input.trim() ? '#d1d5db' : '#ea580c', 
                    color: 'white', 
                    border: 'none', 
                    borderRadius: 10, 
                    fontWeight: 600, 
